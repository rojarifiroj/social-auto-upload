...
def get_title_and_hashtags(filename):
    """
    Get video title and hashtag

    Args:
        filename: Video filename

    Returns:
        Video title and hashtag list
    """

    # Get video title and hashtag txt filename
    print(f"Upload video filename: {filename}")
    ...
    # Get title and hashtag
    splite_str = content.strip().split("\n")

    # Ensure there are enough lines
    if len(splite_str) < 2:
        raise ValueError("TXT file content format is incorrect, must contain title and hashtags")

    # Extract title (all lines except the last line)
    title = "\n".join(splite_str[:-1]).strip()  # Combine all lines as title and remove leading/trailing whitespace

    # Extract tags (last line)
    hashtags_line = splite_str[-1].strip()

    # Process hashtags
    hashtags = hashtags_line.replace("#", "").strip()  # Remove # and leading/trailing whitespace
    hashtags = [tag.strip() for tag in hashtags.split() if tag.strip()]  # Handle empty tags
