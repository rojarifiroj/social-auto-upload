...
class DouYinVideo(object):
    ...
    async def set_schedule_time_douyin(self, page, publish_date):
        if publish_date is None:
            douyin_logger.info("[-] 没有设置定时发布时间，跳过定时发布。")
            return
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        # 将“允许他人保存视频”设置为“不允许”
        douyin_logger.info('[-] 正在设置“允许他人保存视频”为“不允许”...')
        await page.locator('label:has-text("不允许")').click()

        # 将“同步至今日头条”为“是”
        douyin_logger.info('[-] 正在设置“同步至今日头条”为“是”...')
        await page.locator('label:has-text("同步")').nth(1).click()

        # 判断是否需要定时发布
        if self.publish_date:
            await self.set_schedule_time_douyin(page, self.publish_date)
        else:
            douyin_logger.info("[-] 没有设置定时发布时间，视频将立即发布。")
